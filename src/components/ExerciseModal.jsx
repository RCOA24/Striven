...
export default React.memo(function ExerciseModal({ exercise, isOpen, onClose }) {
  ...
  const handleAddToWorkout = async () => {
    ...
    try {
      ...
      toast.success(`✅ ${exercise.name} added to today's workout!`, {
        icon: '💪',
        style: { background: '#10b981', color: 'white' }
      });
    } catch (err) {
      toast.error(err.message || 'Already in today’s workout');
    } finally {
  ...
  const handleToggleFavorite = async () => {
    ...
    try {
      ...
      setIsFav(newFav);
      toast.success(newFav ? '❤️ Saved to favorites!' : '💔 Removed from favorites', {
        style: { background: newFav ? '#ec4899' : '#6b7280', color: 'white' }
      });
    } catch (err) {
    ...
    <>
      ...
      >
        <div
          ref={modalRef}
          tabIndex={-1}
          className="bg-gradient-to-br from-black/95 via-emerald-950/40 to-black/95 rounded-3xl p-6 max-w-4xl w-full border border-emerald-500/30 max-h-[92vh] overflow-y-auto shadow-2xl outline-none scrollbar-hide"
        ...
        >
          ...
          {images.length > 0 && (
            <div className="mb-8" {...swipeHandlers}>
              <div className="relative rounded-2xl overflow-hidden bg-black/40 border border-emerald-500/30">
                ...
                {images.length > 1 && (
                  <>
                    <button onClick={prevImage} className="absolute left-3 top-1/2 -translate-y-1/2 p-3 bg-black/70 hover:bg-emerald-600 rounded-full text-white shadow-xl">
                      <ChevronLeft className="w-6 h-6" />
                    </button>
                    <button onClick={nextImage} className="absolute right-3 top-1/2 -translate-y-1/2 p-3 bg-black/70 hover:bg-emerald-600 rounded-full text-white shadow-xl">
          ...
          <div className="flex gap-4 mt-8">
            <button
              ...
              className="flex-1 bg-gradient-to-r from-emerald-500 to-green-500 hover:from-emerald-400 hover:to-green-400 px-8 py-4 rounded-2xl text-black font-bold text-lg shadow-xl hover:shadow-emerald-500/40 transition-all hover:scale-[1.02] active:scale-98 flex items-center justify-center gap-3 disabled:opacity-70 disabled:cursor-not-allowed"
            >
              {addingToWorkout ? (
            ...
            <button
              ...
              disabled={savingFav}
              className={`px-8 py-4 rounded-2xl font-bold border-2 transition-all hover:scale-105 flex items-center gap-3 min-w-[180px] justify-center ${
                isFav
