...
export const ExerciseItem = ({ ex, index, onRemove }) => {
  ...
  const setsReps = useMemo(() => `${ex.sets || 4}×${ex.reps || 10}`, [ex.sets, ex.reps]);
  const restTime = useMemo(() => `${ex.rest || 90}s`, [ex.rest]);
    ...
    >
      <div 
        className="rounded-xl border border-white/10 bg-zinc-900/70
                   p-2.5 sm:p-3 flex items-center gap-2 sm:gap-2.5 transition-colors active:bg-zinc-900/80"
      >
        ...
        <div
          ...
          className="cursor-grab active:cursor-grabbing p-2 rounded-lg
                     transition-colors active:bg-emerald-500/10 ring-1 ring-transparent active:ring-emerald-500/30
                     touch-manipulation"
        >
        ...
        <div className="relative w-8 h-8 sm:w-9 sm:h-9 flex-shrink-0">
          <div className="absolute inset-0 rounded-full bg-emerald-500 opacity-30 blur-md" />
        ...
        {/* Compact Image */}
        <div className="relative flex-shrink-0 rounded-lg p-[1.5px] bg-emerald-400/30">
          <SafeExerciseImage 
            src={ex.gifUrl} 
            alt={ex.name} 
